function [k_corner,info] = Lcurve(rho,eta,fig,type)
...
if fig == 1 && nargin < 4
	error('The regularization type should pointed out')
end
...
if fig  % Show log-log L-curve and indicate the found corner.
    ...
    loglog(rho, eta, 'b--o','LineWidth', 1.5); hold on; axis square;
    % Mark the corner.
    ...
    if strcmp(type, 'l2-l2')
    	xlabel('log || A x - b ||_2')
    	ylabel('log || x ||_2');
    elseif strcmp(type, 'l2-L2')
    	xlabel('log || A x - b ||_2')
    	ylabel('log || x ||_L2');
    elseif strcmp(type, 'l2-Lx2')
    	xlabel('log || A x - b ||_2')
    	ylabel('log || L x ||_2');
    elseif strcmp(type, 'l2-xHG')
    	xlabel('log || A x - b ||_2')
    	ylabel('log || x ||\_HG');
    elseif strcmp(type, 'xR-xG')
    	xlabel('log || A x - b ||_R')
    	ylabel('log || x ||_G');
    else
    	% pass;
